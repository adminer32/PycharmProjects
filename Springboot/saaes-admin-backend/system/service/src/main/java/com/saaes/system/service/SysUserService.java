...
public class SysUserService extends BaseServiceImpl {
    ...
    public SysUser personalCenterSave(SysUser sysUser) {
        ...
        if (sysUser.getName() != null && !sysUser.getName().trim().isEmpty()) {
            if (sysUser.getName().length() < 4 || sysUser.getName().length() > 20) {
                throw new MyException("用户昵称长度不符合");
            }
            sysUser1.setName(sysUser.getName());
        }

        if (sysUser.getTelephone() != null && !sysUser.getTelephone().trim().isEmpty()) {
            if (!Pattern.matches("^1[3-9]\\d{9}$", sysUser.getTelephone())) {throw new MyException("用户手机号码格式不符合");}
            sysUser1.setTelephone(sysUser.getTelephone());
        }

        if (sysUser.getEmail() != null && !sysUser.getEmail().trim().isEmpty()) {
            if (!Pattern.matches("^[A-Za-z0-9+_.-]+@(.+)$", sysUser.getEmail())) {throw new MyException("用户邮箱地址格式不符合");}
     ...
     * 密码重置
     */
    public String resetPassword(Map<String, Object> param) {

        if (param == null || param.get("userId") == null) {
            return null;
        }

        int length = param.getOrDefault("length", 8) instanceof Integer
                ? (int) param.get("length") : 8;
        int userId = (int) param.get("userId");

        String newPassword = RandomUtil.randomString("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()-_=+", length);
        String pwHash = BCrypt.hashpw(newPassword, BCrypt.gensalt());

        String sql = "update sys_user set password = (:pwHash) where id = (:userId)";
        Map<String, Object> paramMap = Map.of(
                "pwHash", pwHash,
                "userId", userId
        );

        return primaryNPJdbcTemplate.update(sql, paramMap) > 0 ? newPassword : null;
    }
    ...
    public void updateUserPassword(Map<String, Object> param) {
        if (param == null || param.get("oldPassword") == null || param.get("newPassword") == null) {
            throw new MyException("缺少参数");
        }

        int userId = StpUtil.getLoginIdAsInt();
        SysUser sysUser = baseJdbcDao.findById(SysUser.class, userId);
        if (sysUser == null || !sysUser.getPassword().equals(param.get("oldPassword"))) {
            throw new MyException("旧密码不正确");
        }

        String pwHash = BCrypt.hashpw(param.get("newPassword").toString(), BCrypt.gensalt());
