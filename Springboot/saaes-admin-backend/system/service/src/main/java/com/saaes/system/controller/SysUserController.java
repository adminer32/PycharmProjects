...
import com.saaes.system.client.dto.*;
import com.saaes.system.client.entity.SysUser;
import com.saaes.system.client.entity.SysUserGroup;
import com.saaes.system.client.entity.SysUserJob;
import com.saaes.system.client.vo.LoginUserInfoVO;
import com.saaes.system.service.SysLoginService;
import com.saaes.system.service.SysUserService;
...
public class SysUserController {
    ...
    @PostMapping("/login")
    public RestResponse<LoginUserInfoVO> login(HttpServletRequest request, @RequestBody SysLoginRequestDTO sysLoginRequestDTO) {
        return RestResponse.success("登录成功" ,sysLoginService.login(request, sysLoginRequestDTO));
    }

    @SaIgnore
    @Operation(description = "注册")
    @PostMapping("/register")
    public RestResponse register(@RequestBody SysRegisterRequestDTO sysRegisterRequestDTO) {
        sysLoginService.register(sysRegisterRequestDTO);
    ...
    public RestResponse<SysUser> personalCenterSave(@RequestBody SysUser sysUser) {
        return RestResponse.success("更新成功", sysUserService.personalCenterSave(sysUser));
    }

    @Operation(description = "更新用户密码")
    @PostMapping("/updateUserPassword")
    public RestResponse<?> updateUserPassword(@RequestBody Map<String, Object> passwordDate) {
        sysUserService.updateUserPassword(passwordDate);
        return RestResponse.success("更新成功");
    }

    @Operation(description = "获取个人中心详情")
    @GetMapping("/getPersonalCenterDetail")
    public RestResponse<SysUserDTO> getPersonalCenterDetail() {
        SysUser sysUser = sysUserService.getById((Serializable) StpUtil.getLoginId());
        sysUser.setPassword("");
        SysUserDTO sysUserDTO = new SysUserDTO();
        BeanUtils.copyProperties(sysUser, sysUserDTO);
        return RestResponse.success("查询成功", sysUserDTO);
    }
    ...
    @SaCheckPermission(value = {"system:user:resetPassword"})
    @Operation(description = "密码重置")
    @PostMapping("/resetPassword")
    public RestResponse<?> resetPassword(@RequestBody Map<String, Object> params) {
        String result = sysUserService.resetPassword(params);
        Map<String, Object> map = new HashMap<>();
        map.put("newPassword", result);
        return result != null ? RestResponse.success(map) : RestResponse.error();
    }
